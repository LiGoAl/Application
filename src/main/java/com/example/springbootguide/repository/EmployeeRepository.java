...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query(value = "SELECT * FROM department d ORDER BY (SELECT COUNT(*) FROM employee e WHERE e.department_id = d.id) DESC LIMIT 1", nativeQuery = true)
    Department findDepartmentWithMostEmployees();

    @Query(value = "SELECT * FROM department d ORDER BY (SELECT COUNT(*) FROM employee e WHERE e.department_id = d.id) LIMIT 1", nativeQuery = true)
    ...
    @Query(value = "SELECT d.*, AVG(e.salary) AS avg_salary FROM department d " +
            ...
            "ORDER BY avg_salary DESC LIMIT 10", nativeQuery = true)
